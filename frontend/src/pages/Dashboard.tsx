  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-3.5 pb-3 flex items-center justify-between relative">
        ...
        <span
          className="text-[9px] px-2 py-0.5 rounded-full font-mono relative z-10"
          style={{ background: `${s.hex}14`, border: `1px solid ${s.hex}44`, color: s.hex }}
        >
          {s.hex === '#34D399' ? 'READY' : s.hex === '#FBBF24' ? 'SOLD' : s.hex === '#F87171' ? 'HOLD' : 'STOCK'}
        </span>
      </div>

      {/* ── Panel angka (terang, ala referensi) ── */}
      <div
        className="mx-2.5 mb-2.5 rounded-lg px-4 py-3.5 relative overflow-hidden"
        style={{
          background: `linear-gradient(150deg, ${s.hex}12, rgba(24,29,38,0.92) 55%, rgba(18,22,30,0.96))`,
          border: '1px solid rgba(148,163,184,0.1)'
        }}
      >
        {/* corner glow */}
        <div
          className="absolute -top-8 -right-8 w-24 h-24 rounded-full blur-2xl opacity-20 pointer-events-none transition-opacity group-hover:opacity-40"
          style={{ background: s.hex }}
        />

        <p className="text-[10px] text-slate-500 tracking-wide relative z-10">{s.sub}</p>
        <p className="text-[26px] font-black font-mono leading-tight relative z-10">
          <span className="text-white">{n.toLocaleString('id-ID')}</span>
          <span className="text-slate-500"> akun</span>
        </p>

        {/* aksi kecil ala Send/Request */}
        <div className="flex items-center gap-4 mt-2.5 relative z-10">
          <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-slate-400 group-hover:text-slate-200 transition-colors">
            <span className="w-4 h-4 rounded-full flex items-center justify-center" style={{ background: `${s.hex}1c`, border: `1px solid ${s.hex}55` }}>
              <ArrowUpRight size={9} style={{ color: s.hex }} />
            </span>
            Detail
          </span>
          <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-slate-400 group-hover:text-slate-200 transition-colors">
            <span className="w-4 h-4 rounded-full flex items-center justify-center" style={{ background: 'rgba(148,163,184,0.12)', border: '1px solid rgba(148,163,184,0.3)' }}>
